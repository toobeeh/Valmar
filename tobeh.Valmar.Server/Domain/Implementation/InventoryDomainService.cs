    ...
    BubbleChunkTreeProvider bubbleChunks) : IInventoryDomainService
{
    ...
    public async Task BuySprite(MemberDdo member, int spriteId)
    {
        ...
        if (sprite.RequiredFlags.Count > 0 && sprite.RequiredFlags.All(flag => !member.MappedFlags.Contains(flag)))
        {
    ...
    public Task<double> GetNextSlotRemainingDrops(MemberDdo member)
    {
        ...
        var remainingDrops = member.Drops - InventoryHelper.GetRequiredDropsForNextSlot(baseSlots + 1);
