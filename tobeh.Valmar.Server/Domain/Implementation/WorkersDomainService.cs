    ...
    PalantirContext db) : IWorkersDomainService
{
    ...
    public async Task<LobbyBotInstanceEntity> GetUnclaimedInstanceForServer()
    {
        ...
        var validMemberClaims = memberClaims.Where(claim => memberPermissions[claim.Login]).Select(claim => claim.InstanceId).ToList();
    ...
    public async Task<LobbyBotInstanceEntity> ClaimInstanceForWorker(string workerUlid, int instanceId, string claimUlid, string? lastClaimUlid = null)
    {
    ...
    public async Task<LobbyBotOptionEntity> UpdateGuildOptions(long guildId, string name, string prefix, long? channelId = null)
    {
