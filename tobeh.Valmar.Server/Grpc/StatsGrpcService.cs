...
{
    public override async Task<BubbleTimespanRangeReply> GetBubbleTimespanRange(BubbleTimespanRangeRequest request,
    ...
    public override async Task<BubbleProgressMessage> GetBubbleProgress(GetBubbleProgressMessage request,
        ServerCallContext context)
    {
        logger.LogTrace("GetBubbleProgress(request={request})", request);

        var range = await statsService.GetBubbleProgress(request.Login, request.StartDate.ToDateTimeOffset(),
            request.EndDate.ToDateTimeOffset(),
    ...
    public override async Task<LeaderboardMessage> GetLeaderboard(GetLeaderboardMessage request,
        ServerCallContext context)
    {
        logger.LogTrace("GetLeaderboard(request={request})", request);

        var members = request.GuildId is { } id
            ? await membersService.GetGuildMembers(id)
